...
function ArchitectureNodeComponent({ id, data }: NodeProps<ArchitectureNodeData>) {
  ...
  return (
    <div
      className={clsx(
        "architecture-node group overflow-hidden rounded-xl shadow-node transition duration-150",
    ...
    >
      ...
      <div className="rounded-lg m-1.5 p-3 bg-black/15 backdrop-blur-sm">
        <NodeHeader
          name={name}
          type={type}
          color={colors.text}
        ...
        {isCluster ? (
          <>
            ...
            <div className="architecture-node__meta mt-3 flex items-center gap-2 text-[11px] text-white/70">
              <span className="rounded px-2 py-1 font-medium bg-white/10">
                {data.cluster.componentCount} blocks
              </span>
              {childCount > 0 && (
                <span className="rounded px-2 py-1 font-medium bg-white/15 text-white">
                  click to expand
                </span>
              )}
            </div>
            {data.cluster.typeBreakdown && Object.keys(data.cluster.typeBreakdown).length > 1 && (
              <div className="mt-2 flex flex-wrap gap-1">
                {Object.entries(data.cluster.typeBreakdown).map(([t, count]) => (
                  <span key={t} className="rounded px-1.5 py-0.5 text-[9px] font-medium bg-white/10 text-white/70">
                    {t}: {count}
                  </span>
                ))}
              </div>
